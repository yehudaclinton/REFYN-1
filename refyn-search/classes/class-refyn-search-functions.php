...
class Refyn_Search_Functions
{
	public static function symbol_entities() {
		...
		return apply_filters( 'refyn_symbol_entities', $symbol_entities );
	}
	...
	public static function special_characters_list() {
		...
		return apply_filters( 'refyn_special_characters', $special_characters );
	}
	public static function remove_special_characters_in_mysql( $field_name ) {
		if ( trim( $field_name ) == '' ) return '';
		$refyn_search_remove_special_character = get_option( 'refyn_search_remove_special_character', 'no' );
		if ( 'no' == $refyn_search_remove_special_character ) {
			return $field_name;
		}
		$refyn_search_special_characters = get_option( 'refyn_search_special_characters', array() );
		if ( !is_array( $refyn_search_special_characters ) || count( $refyn_search_special_characters ) < 1 ) {
			return $field_name;
		}
		foreach ( $refyn_search_special_characters as $special_symbol ) {
	...
	public static function refyn_limit_words($str='',$len=100,$more) {
		...
		if ( class_exists('SitePress') ) {
			...
			if ( is_array($active_languages)  && count($active_languages) > 0 ) {
				...
				foreach ( $active_languages as $language ) {
					...
					Refyn_Search_Functions::create_page_wpml( $trid, $language['code'], $source_lang_code, $slug.'-'.$language['code'], $page_title.' '.$language['display_name'], $page_content );
				}
			}
		}
	}
	...
	public static function add_rewrite_rules( $aRules ) {
		global $wpdb;
		$shortcode   = 'refyn_search';
		$option_name = 'refyn_search_page_id';
	...
	public static function get_product_thumbnail_url( $post_id, $size = 'shop_catalog', $placeholder_width = 0, $placeholder_height = 0  ) {
		...
		if ( trim( $mediumSRC == '' ) ) {
			$mediumSRC = ( ( version_compare( $woocommerce_db_version, '2.1', '<' ) && null !== $woocommerce_db_version ) ? refyn_placeholder_img_src() : wc_placeholder_img_src() );
		}
		return $mediumSRC;
	}
}
