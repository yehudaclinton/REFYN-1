...
class REFYN_Posts_Data
{
	...
	/**
	 * Refyn Search Post Table - set table name
	 *
	 * @return void
	 */
	...
	/**
	 * Refyn Search Post Table - return sql
	 *
	 * @return void
	 */
	public function get_sql( $search_keyword = '', $search_keyword_nospecial = '', $post_type = 'product', $number_row, $start = 0, $check_existed = false ) {
		...
		$groupby = array();
		$orderby = array();
		$items_excluded = $refyn_exclude_data->get_array_items( $post_type );
		if ( 'page' == $post_type ) {
			global $refyn_search_page_id;
			$items_excluded = array_merge( array( (int) $refyn_search_page_id ), $items_excluded );
		}
		...
		$where_title = ' ( ';
		$where_title .= $wpdb->prepare( Refyn_Search_Functions::remove_special_characters_in_mysql( 'pp.post_title' ) . " LIKE '%s' OR " . Refyn_Search_Functions::remove_special_characters_in_mysql( 'pp.post_title' ) . " LIKE '%s' ", $search_keyword.'%', '% '.$search_keyword.'%' );
		if ( '' != $search_keyword_nospecial ) {
			$where_title .= " OR ". $wpdb->prepare( Refyn_Search_Functions::remove_special_characters_in_mysql( 'pp.post_title' ) . " LIKE '%s' OR " . Refyn_Search_Functions::remove_special_characters_in_mysql( 'pp.post_title' ) . " LIKE '%s' ", $search_keyword_nospecial.'%', '% '.$search_keyword_nospecial.'%' );
		}
		$search_keyword_no_s_letter = Refyn_Search_Functions::remove_s_letter_at_end_word( $search_keyword );
		if ( $search_keyword_no_s_letter != false ) {
			$where_title .= " OR ". $wpdb->prepare( Refyn_Search_Functions::remove_special_characters_in_mysql( 'pp.post_title' ) . " LIKE '%s' OR " . Refyn_Search_Functions::remove_special_characters_in_mysql( 'pp.post_title' ) . " LIKE '%s' ", $search_keyword_no_s_letter.'%', '% '.$search_keyword_no_s_letter.'%' );
		}
		$where_title .= ' ) ';
		$where['search']   = array();
...
$refyn_posts_data = new REFYN_Posts_Data();
