...
/* Copyright 2012 REFYN  This software is distributed under the terms of GNU GENERAL PUBLIC LICENSE Version 3, 29 June 2007 */
...
class Refyn_Search_Admin_Interface extends Refyn_Search_Admin_UI
...

{
	...
	public function admin_script_load() {
		...
		wp_register_script( 'refynrev-chosen-new', $this->admin_plugin_url() . '/assets/js/chosen/chosen.jquery' . $suffix . '.js', array( 'jquery' ), true, false );



		wp_register_script( 'refynrev-style-checkboxes', $this->admin_plugin_url() . '/assets/js/iphone-style-checkboxes.js', array('jquery'), true, false );
		...
		wp_register_script( 'refynrev-admin-ui-script', $this->admin_plugin_url() . '/assets/js/admin-ui-script.js', array('jquery'), true, true );



		wp_register_script( 'refynrev-typography-preview', $this->admin_plugin_url() . '/assets/js/refynrev-typography-preview.js',  array('jquery'), false, true );



		wp_register_script( 'refynrev-settings-preview', $this->admin_plugin_url() . '/assets/js/refynrev-settings-preview.js',  array('jquery'), false, true );
		...
		wp_enqueue_script( 'refynrev-chosen-new' );



		wp_enqueue_script( 'refynrev-style-checkboxes' );



		wp_enqueue_script( 'refynrev-admin-ui-script' );



		wp_enqueue_script( 'refynrev-typography-preview' );



		wp_enqueue_script( 'refynrev-settings-preview' );
	...
	public function admin_css_load () {
		...
		wp_enqueue_style( 'refynrev-admin-ui-style', $this->admin_plugin_url() . '/assets/css/admin-ui-style' . $suffix . '.css' );
		...
		if ( version_compare( $wp_version, '3.8', '>=' ) ) {
			...
			wp_enqueue_style( 'refynrev-admin-flat-ui-style', $this->admin_plugin_url() . '/assets/css/admin-flat-ui-style' . $suffix . '.css' );
		...
		wp_enqueue_style( 'refynrev-chosen-new-style', $this->admin_plugin_url() . '/assets/js/chosen/chosen' . $suffix . '.css' );



		wp_enqueue_style( 'refynrev-tiptip-style', $this->admin_plugin_url() . '/assets/js/tipTip/tipTip.css' );
	...
	public function get_success_message( $message = '' ) {
		...
		if ( trim( $message ) == '' ) $message = __( 'Settings successfully saved.' , 'refyn' ); 
	...
	public function get_error_message( $message = '' ) {
		...
		if ( trim( $message ) == '' ) $message = __( 'Error: Settings can not save.' , 'refyn' ); 
	...
	public function get_reset_message( $message = '' ) {
		...
		if ( trim( $message ) == '' ) $message = __( 'Settings successfully reseted.' , 'refyn' ); 
	...
	public function get_font_weights() {
		...
		$font_weights = array (
			...
			'300 italic'		=> __( 'Thin/Italic', 'refyn' ),



			'normal'			=> __( 'Normal', 'refyn' ),



			'italic'			=> __( 'Italic', 'refyn' ),
			...
			'bold italic'		=> __( 'Bold/Italic', 'refyn' ),
	...
	public function get_border_styles() {
		...
		$border_styles = array (
			...
			'solid'				=> __( 'Solid', 'refyn' ),



			'double'			=> __( 'Double', 'refyn' ),



			'dashed'			=> __( 'Dashed', 'refyn' ),



			'dotted'			=> __( 'Dotted', 'refyn' ),



			'groove'			=> __( 'Groove', 'refyn' ),



			'ridge'				=> __( 'Ridge', 'refyn' ),



			'inset'				=> __( 'Inset', 'refyn' ),



			'outset'			=> __( 'Outset', 'refyn' ),
...
$all_settings=get_option('refyn_all_settings');
		...
		update_option('refyn_all_settings', $all_settings);
	...
	public function admin_forms( $options, $form_key, $option_name = '', $form_messages = array() ) {
		...
		global $refyn_search_fonts_face, $refyn_search_uploader, $current_subtab, $setting_options;
		...
		<div class="refynrev_panel_container" style="visibility:hidden; height:0; overflow:hidden;" >
		...
		foreach ( $options as $value ) {
			...
			if ( $tip && in_array( $value['type'], array( 'checkbox' ) ) ) {
				...
				$tip = '<div class="help_tip refyn-plugin-ui-icon refyn-plugin-ui-help-icon" data-tip="' . esc_attr( $tip ) . '"></div>';
			...
			// Determine if a FREE or PREMIUM user



		   $user_data = file_get_contents($remote_API_server.'?siteurl='.$home_url.'&apikey='.$api_key.'&r=user' );



		   $get_user_data = json_decode($user_data, true);



		   //echo "G="; print_r ($get_user_data);



		   $isUserLevelFree = isset($get_user_data['usage_exceed']) ? $get_user_data['usage_exceed'] : 1;
			...
			switch( $value['type'] ) {
				...
				case 'heading':
					...
					echo '<div id="'. esc_attr( $value['id'] ) . '" class="refynrev_panel_inner '. esc_attr( $value['class'] ) .'" style="'. esc_attr( $value['css'] ) .'">' . "\n\n";
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							...
							<input
								...
								class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?> <?php echo esc_attr( $value['class'] ); ?>"
				...
				case 'multiselect' :
					...
					?><tr valign="top">
						...
						<th scope="row" class="titledesc">
							...
							<label for="<?php echo $id_attribute; ?>"><?php echo esc_html( $value['name'].$setting_options['custom_formula'] ); ?></label>
							...
							<fieldset>
								...
								if ( is_array( $value['onoff_options'] ) && count( $value['onoff_options'] ) > 0 ) {
									...
									foreach ( $value['onoff_options'] as $i_option ) {
										...
										if ( ! isset( $i_option['checked_label'] ) ) $i_option['checked_label'] = __( 'ON', 'refyn' );



										if ( ! isset( $i_option['unchecked_label'] ) ) $i_option['unchecked_label'] = __( 'OFF', 'refyn' );
										...
										<li>
                                            ...
                                            <input
                                                ...
                                                class="refynrev-ui-onoff_radio <?php echo esc_attr( $value['class'] ); ?>"
				...
				case 'onoff_checkbox' :
					...
					if ( ! isset( $value['checked_label'] ) ) $value['checked_label'] = __( 'ON', 'refyn' );



					if ( ! isset( $value['unchecked_label'] ) ) $value['unchecked_label'] = __( 'OFF', 'refyn' );
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							...
							<input
								...
								class="refynrev-ui-onoff_checkbox <?php echo esc_attr( $value['class'] ); ?>"
				...
				case 'image_size' :
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
							...
							<label><?php _e( 'Width', 'refyn' ); ?> <input name="<?php echo $name_attribute; ?>[width]" id="<?php echo $id_attribute; ?>-width" type="text" class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?>-width" value="<?php echo $width; ?>" /></label>
							...
							<label><?php _e( 'Height', 'refyn' ); ?> <input name="<?php echo $name_attribute; ?>[height]" id="<?php echo $id_attribute; ?>-height" type="text" class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?>-height" value="<?php echo $height; ?>" /></label>
							...
							<label><?php _e( 'Hard Crop', 'refyn' ); ?> <input name="<?php echo $name_attribute; ?>[crop]" id="<?php echo $id_attribute; ?>-crop" type="checkbox" class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?>-crop" <?php echo $crop; ?> /></label>
				...
				case 'single_select_page' :
					...
					$args = array( 'name'				=> $name_attribute,
								   ...
								   'class'				=> 'refynrev-ui-' . sanitize_title( $value['type'] ) . ' ' . $value['class'],
				...
				case 'typography':
					...
					?><tr valign="top">
						...
						<td class="forminp">
                            ...
                            <div class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?>-control">
							...
							<select
								...
								class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?>-size chzn-select"
                           ...
                           <!-- Font Face -->
							...
							<select
								...
								class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?>-face chzn-select"
								...
								<optgroup label="<?php _e( '-- Default Fonts --', 'refyn' ); ?>">
                                ...
                                <?php
									...
									foreach ( $refyn_search_fonts_face->get_default_fonts() as $val => $text ) {
                                ...
                                <optgroup label="<?php _e( '-- Google Fonts --', 'refyn' ); ?>">
                                ...
                                <?php
									...
									foreach ( $refyn_search_fonts_face->get_google_fonts() as $font ) {
                           ...
                           <select
								...
								class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?>-style chzn-select"
                           ...
                           <input
								...
								class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?>-color refynrev-color-picker"
                           ...
                           <div class="refynrev-ui-typography-preview"><a href="#" class="refynrev-ui-typography-preview-button button submit-button" title="<?php _e( 'Preview your customized typography settings', 'refyn'); ?>"><span>&nbsp;</span></a></div>
				...
				case 'border':
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-border_corner">
                            ...
                            <div class="refynrev-ui-settings-control">
							...
							<select
								...
								class="refynrev-ui-border_styles-width chzn-select"
                           ...
                           <select
								...
								class="refynrev-ui-border_styles-style chzn-select"
                           ...
                           <input
								...
								class="refynrev-ui-border_styles-color refynrev-color-picker"
                           ...
                           <div class="refynrev-ui-settings-preview"><a href="#" class="refynrev-ui-border-preview-button refynrev-ui-settings-preview-button button submit-button" title="<?php _e( 'Preview your customized border settings', 'refyn'); ?>"><span>&nbsp;</span></a></div>
                           ...
                           <!-- Border Corner : Rounded or Square -->
								...
								<input
                                    ...
                                    class="refynrev-ui-border-corner refynrev-ui-onoff_checkbox <?php echo esc_attr( $value['class'] ); ?>"



                                    checked_label="<?php _e( 'Rounded', 'refyn' ); ?>"



                                    unchecked_label="<?php _e( 'Square', 'refyn' ); ?>"
							...
							<!-- Border Rounded Value -->
								...
								<div class="refynrev-ui-border-corner-value-container">



                                	<div class="refynrev-ui-border_corner-top_left">



                                        <span class="refynrev-ui-border_corner-span"><?php _e( 'Top Left Corner', 'refyn' ); ?></span>



                                        <div class="refynrev-ui-slide-container">



                                            <div class="refynrev-ui-slide-container-start">



                                                <div class="refynrev-ui-slide-container-end">



                                                    <div class="refynrev-ui-slide" id="<?php echo $id_attribute; ?>-top_left_corner_div" min="<?php echo esc_attr( $value['min'] ); ?>" max="<?php echo esc_attr( $value['max'] ); ?>" inc="<?php echo esc_attr( $value['increment'] ); ?>"></div>
                                            ...
                                            <div class="refynrev-ui-slide-result-container">
                                            ...
                                            <input
                                                ...
                                                class="refynrev-ui-border_top_left_corner refynrev-ui-slider"



                                            /> <span class="refynrev-ui-border_corner-px">px</span>
                                    ...
                                    <div class="refynrev-ui-border_corner-top_right">



                                        <span class="refynrev-ui-border_corner-span"><?php _e( 'Top Right Corner', 'refyn' ); ?></span> 
                                        ...
                                        <div class="refynrev-ui-slide-container">
                                            ...
                                            <div class="refynrev-ui-slide-container-start">
                                                ...
                                                <div class="refynrev-ui-slide-container-end">
                                                    ...
                                                    <div class="refynrev-ui-slide" id="<?php echo $id_attribute; ?>-top_right_corner_div" min="<?php echo esc_attr( $value['min'] ); ?>" max="<?php echo esc_attr( $value['max'] ); ?>" inc="<?php echo esc_attr( $value['increment'] ); ?>"></div>
                                            ...
                                            <input
                                                ...
                                                class="refynrev-ui-border_top_right_corner refynrev-ui-slider"
                                    ...
                                    <div class="refynrev-ui-border_corner-bottom_right">



                                        <span class="refynrev-ui-border_corner-span"><?php _e( 'Bottom Right Corner', 'refyn' ); ?></span> 
                                        ...
                                        <div class="refynrev-ui-slide-container">
                                            ...
                                            <div class="refynrev-ui-slide-container-start">
                                                ...
                                                <div class="refynrev-ui-slide-container-end">
                                                    ...
                                                    <div class="refynrev-ui-slide" id="<?php echo $id_attribute; ?>-bottom_right_corner_div" min="<?php echo esc_attr( $value['min'] ); ?>" max="<?php echo esc_attr( $value['max'] ); ?>" inc="<?php echo esc_attr( $value['increment'] ); ?>"></div>
                                            ...
                                            <input
                                                ...
                                                class="refynrev-ui-border_bottom_right_corner refynrev-ui-slider"
                                    ...
                                    <div class="refynrev-ui-border_corner-bottom_left">



                                        <span class="refynrev-ui-border_corner-span"><?php _e( 'Bottom Left Corner', 'refyn' ); ?></span>
                                        ...
                                        <div class="refynrev-ui-slide-container"> 
                                            ...
                                            <div class="refynrev-ui-slide-container-start">
                                                ...
                                                <div class="refynrev-ui-slide-container-end">
                                                    ...
                                                    <div class="refynrev-ui-slide" id="<?php echo $id_attribute; ?>-bottom_left_corner_div" min="<?php echo esc_attr( $value['min'] ); ?>" max="<?php echo esc_attr( $value['max'] ); ?>" inc="<?php echo esc_attr( $value['increment'] ); ?>"></div>
                                            ...
                                            <input
                                                ...
                                                class="refynrev-ui-border_bottom_left_corner refynrev-ui-slider"
				...
				case 'border_styles':
					...
					?><tr valign="top">
						...
						<td class="forminp">
                           ...
                           <div class="refynrev-ui-settings-preview"><a href="#" class="refynrev-ui-border-preview-button refynrev-ui-settings-preview-button button submit-button" title="<?php _e( 'Preview your customized border styles settings', 'refyn'); ?>"><span>&nbsp;</span></a></div>
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-box_shadow">
                            ...
                            <input
                                    ...
                                    class="refynrev-ui-box_shadow-enable refynrev-ui-onoff_checkbox <?php echo esc_attr( $value['class'] ); ?>"



                                    checked_label="<?php _e( 'YES', 'refyn' ); ?>"



                                    unchecked_label="<?php _e( 'NO', 'refyn' ); ?>"
                            ...
                            <div class="refynrev-ui-box_shadow-enable-container">
							...
							<select
								...
								class="refynrev-ui-box_shadow-h_shadow chzn-select"



                                data-placeholder="<?php _e( 'Horizontal Shadow', 'refyn' ); ?>"
							...
							<select
								...
								class="refynrev-ui-box_shadow-v_shadow chzn-select"



                                data-placeholder="<?php _e( 'Vertical Shadow', 'refyn' ); ?>"
                           ...
                           <!-- Box Blur Distance -->
							...
							<select
								...
								class="refynrev-ui-box_shadow-blur chzn-select"



                                data-placeholder="<?php _e( 'Blur Distance', 'refyn' ); ?>"
                           ...
                           <!-- Box Spread -->
							...
							<select
								...
								class="refynrev-ui-box_shadow-spread chzn-select"



                                data-placeholder="<?php _e( 'Spread Size', 'refyn' ); ?>"
                           ...
                           <!-- Box Shadow Inset -->
                                ...
                                <input
                                    ...
                                    class="refynrev-ui-box_shadow-inset refynrev-ui-onoff_checkbox"



                                    checked_label="<?php _e( 'INNER', 'refyn' ); ?>"



                                    unchecked_label="<?php _e( 'OUTER', 'refyn' ); ?>"
                           ...
                           <input
								...
								class="refynrev-ui-box_shadow-color refynrev-color-picker"
                           ...
                           <div class="refynrev-ui-settings-preview"><a href="#" class="refynrev-ui-box_shadow-preview-button refynrev-ui-settings-preview-button button submit-button" title="<?php _e( 'Preview your customized box shadow settings', 'refyn'); ?>"><span>&nbsp;</span></a></div>   
				...
				case 'slider':
					...
					?><tr valign="top">
                        ...
                        <div class="refynrev-ui-slide-container">
                            ...
                            <div class="refynrev-ui-slide-container-start"><div class="refynrev-ui-slide-container-end">



                                <div class="refynrev-ui-slide" id="<?php echo $id_attribute; ?>_div" min="<?php echo esc_attr( $value['min'] ); ?>" max="<?php echo esc_attr( $value['max'] ); ?>" inc="<?php echo esc_attr( $value['increment'] ); ?>"></div>
				...
				case 'upload':
					...
					$class = 'refynrev-ui-' . sanitize_title( $value['type'] ) . ' ' . esc_attr( $value['class'] );
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
                        	...
                        	<?php echo $refyn_search_uploader->upload_input( $name_attribute, $id_attribute, $option_value, $value['default'], esc_html( $value['name'] ), $class, esc_attr( $value['css'] ) , '' , '', $isUserLevelFree);?>
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
                        	...
                        	<?php wp_editor( 	$option_value, 
												...
												array( 	'textarea_name' => $name_attribute, 
														...
														'editor_class'	=> 'refynrev-ui-' . sanitize_title( $value['type'] ) . ' ' . esc_attr( $value['class'] ), 
				...
				case 'array_textfields':
					...
					?><tr valign="top">
						...
						<td class="forminp forminp-<?php echo sanitize_title( $value['type'] ) ?>">
                        	...
                        	<div class="refynrev-ui-array_textfields-container">
							...
							?>
                                ...
                                <label><input
                                    ...
                                    class="refynrev-ui-<?php echo sanitize_title( $value['type'] ) ?> <?php echo esc_attr( $text_field['class'] ); ?>"
				...
				jQuery(document).ready(function(){
					...
					jQuery('.edit_stopword').click(function(){
						...
						jQuery('.new_stopword_'+id).show();



						jQuery('.save_value_'+id).show();



						jQuery('.stopwords_value_'+id).hide();



						jQuery('.cancel_'+id).show();
					...
					jQuery('.cancel_stopword').click(function(){
						...
						jQuery('.new_stopword_'+id).hide();



						jQuery('.save_value_'+id).hide();



						jQuery('.stopwords_value_'+id).show();
					...
					jQuery('.save_stopword').click(function(){
					...
					jQuery.ajax({
           			 ...
           			 url: '<?php echo admin_url('admin-ajax.php'); ?>?action=stopword_modification_request', 
            		 ...
            		 type: 'POST',
	   				 ...
	   				 data: { id: jQuery(this).attr('data-id'), stopword: jQuery('#new_stopword_'+id).val() },
            		...
            		success: function(response){   
						...
						if(response.result >= 1){
							...
							alert('some problem occured in change');
			...
			jQuery('.delete_stopword').click(function(){
					...
					jQuery.ajax({
           			 ...
           			 url: '<?php echo admin_url('admin-ajax.php'); ?>?action=stopword_delete_request', 
                ...
                <tr><th>S.No.</th><th>Stopwords</th><th>Options</th></tr></thead>
				...
				if(!empty($_POST['stopword'])){
					...
					$insert_stopword = file_get_contents($remote_API_server.'?siteurl='.$home_url.'&apikey='.$api_key.'&r=insert_stopword&stopword='.sanitize_text_field( $_POST['stopword'] ));



  				$inserted_stopword = json_decode($insert_stopword, true);
				...
				$all_stopwords = file_get_contents($remote_API_server.'?siteurl='.$home_url.'&apikey='.$api_key.'&r=get_all_stopwords_count' );



  				$get_stopword_list = json_decode($all_stopwords, true);



				$rec_count=$get_stopword_list['total_count'];
				...
				$left_rec = $rec_count - ($page1 * $rec_limit);



				$all_stopwords1 = file_get_contents($remote_API_server.'?siteurl='.$home_url.'&apikey='.$api_key.'&r=get_all_stopwords&offset='.$offset.'&rec_limit='.$rec_limit );



  				$get_stopword_list1 = json_decode($all_stopwords1, true);
		 		...
		 		// Dont show for free / expired user
...

		 		{
		 			...
		 			<h1>Premium Only. Sample</h1>


		 			<a href="<? echo plugin_dir_url( __FILE__ ); ?>3-stopwords-edited.png">	


		 			<img src="<? echo plugin_dir_url( __FILE__ ); ?>3-stopwords-edited-375x400.png"></a>
...

		 		{
				  ...
				  foreach($get_stopword_list1['result'] as $list){			
				  ...
				  <tr><td><?php echo $counter; ?></td><td><span class="stopwords_value_<?php echo $list['idstopwords']; ?>"><?php echo $list['stopword']; ?></span>
				  ...
				  <input class="new_stopword_<?php echo $list['idstopwords']; ?>" type="text" id="new_stopword_<?php echo $list['idstopwords']; ?>" name="new_stopword_<?php echo $list['idstopwords']; ?>" value="<?php echo $list['stopword']; ?>" style="display:none;"/>
				  ...
				  <a class="save_value_<?php echo $list['idstopwords']; ?> save_stopword" href="javascript:void(0);" style="display:none;" data-id="<?php echo $list['idstopwords']; ?>">Save</a>



				   <a class="cancel_<?php echo $list['idstopwords']; ?> cancel_stopword" href="javascript:void(0);" style="display:none;" data-id="<?php echo $list['idstopwords']; ?>">Cancel</a></td>
				  ...
				  <td><a class="edit_stopword" href="javascript:void(0);" data-id="<?php echo $list['idstopwords']; ?>" >Edit</a>&nbsp;&nbsp;&nbsp; 



					<a class="delete_stopword" href="javascript:void(0);" data-id="<?php echo $list['idstopwords']; ?>">Delete</a></td></tr>
            ...
            echo "<a href = \"".home_url()."/wp-admin/admin.php?page=woo-refyn-search&tab=stopwords&page1=$last\"> Previous </a> |";



            echo "<a href = \"".home_url()."/wp-admin/admin.php?page=woo-refyn-search&tab=stopwords&page1=$page1\"> Next  </a>";
            ...
            echo "<a href = \"".home_url()."/wp-admin/admin.php?page=woo-refyn-search&tab=stopwords&page1=$last\"> Previous </a>";
               ...
               <span>Add New Stopword </span> <input type="text" name="stopword" id="stopword" value="" <? if ($isUserLevelFree) echo "readonly";  ?> />
				...
				jQuery(document).ready(function(){
					...
					jQuery('.save_keyword').click(function(){
					...
					jQuery.ajax({
           			 ...
           			 url: '<?php echo admin_url('admin-ajax.php'); ?>?action=keyword_modification_request', 
            		 ...
            		 type: 'POST',
	   				 ...
	   				 data: { id: jQuery(this).attr('data-id'), seo: jQuery('#new_seo_'+id).val(), times: jQuery('#new_times_'+id).val(), new_the_kw_user_selected: jQuery('#new_the_kw_user_selected_'+id).val() },
			...
			jQuery('.delete_keyword').click(function(){
					...
					jQuery.ajax({
           			 ...
           			 url: '<?php echo admin_url('admin-ajax.php'); ?>?action=keyword_delete_request', 
				...
				jQuery( ".keyword_status_change" ).change(function() {
					...
					jQuery.ajax({
           			 ...
           			 url: '<?php echo admin_url('admin-ajax.php'); ?>?action=keyword_change_status', 
            		 ...
            		 type: 'POST',
	   				 ...
	   				 data: { id: id, option: jQuery(this).val()  },
                ...
                <tr><th>S.No.</th><th>Keyword</th><th>Times Used</th><th>SEO Keyword</th><th>Keyword User Selected</th><th>Options</th></tr></thead>
				...
				$all_keywords = file_get_contents($remote_API_server.'?siteurl='.$home_url.'&apikey='.$api_key.'&r=get_all_keywords_count' );



  				$get_keyword_list = json_decode($all_keywords, true);



				$rec_count=$get_keyword_list['total_count'];
				...
				$all_keywords = file_get_contents($remote_API_server.'?siteurl='.$home_url.'&apikey='.$api_key.'&r=get_all_keywords&offset='.$offset.'&rec_limit='.$rec_limit );



				//echo "all_keywords="; print_r($all_keywords);



  				$get_stopword_list = json_decode($all_keywords, true);
				...
				foreach($get_stopword_list['result'] as $list){



					$kw = preg_replace('/\s+/', '_', $list['kw']);			
                ...
                <tr><td><?php echo $counter; ?></td><td><span class="keyword_value_<?php echo $kw; ?>"><?php echo $list['kw']; ?></span>
                 ...
                 <td><span class="stopwords_value_<?php echo $kw; ?>"><?php echo ( !empty($list['times']) )? $list['times'] : '__'; ?></span>



                  <input class="new_stopword_<?php echo $kw; ?>" type="text" id="new_times_<?php echo $kw; ?>" name="new_times_<?php echo $kw; ?>" value="<?php echo $list['times']; ?>" style="display:none;width:90%;"/></td>
                 ...
                 <td><span class="stopwords_value_<?php echo $kw; ?>"><?php echo ( !empty($list['seo']) ) ? $list['seo'] : '__'; ?></span>



                  <input class="new_stopword_<?php echo $kw; ?>" type="text" id="new_seo_<?php echo $kw; ?>" name="new_seo_<?php echo $kw; ?>" value="<?php echo $list['seo']; ?>" style="display:none;width:90%;;"/></td>
                 ...
                 <td><span class="stopwords_value_<?php echo $kw; ?>"><?php echo ( !empty($list['the_kw_user_selected']) )? $list['the_kw_user_selected'] : '__'; ?></span>



                  <input class="new_stopword_<?php echo $kw; ?>" type="text" id="new_the_kw_user_selected_<?php echo $kw; ?>" name="new_the_kw_user_selected_<?php echo $kw; ?>" value="<?php echo $list['the_kw_user_selected']; ?>" style="display:none;width:90%;"/>



                  <a class="save_value_<?php echo $kw; ?> save_keyword" href="javascript:void(0);" style="display:none;" data-id="<?php echo $list['kw']; ?>" data-slug="<?php echo $kw; ?>">Save</a>



                 <a class="cancel_<?php echo $kw; ?> cancel_stopword" href="javascript:void(0);" style="display:none;" data-id="<?php echo $kw; ?>">Cancel</a></td>
                ...
                <a class="edit_stopword" href="javascript:void(0);" data-id="<?php echo $kw; ?>" >Edit</a>&nbsp;&nbsp;&nbsp; 



                  <a class="delete_keyword" href="javascript:void(0);" data-id="<?php echo $list['kw']; ?>">Delete</a>



                <? } // end of if isUserLevelFree
                ...
                ?>  
                  ...
                  <select class="keyword_status_change" data-id="<?php echo $list['kw']; ?>">
                  ...
                  <option <? if ($isUserLevelFree) echo 'disabled="disabled"';  ?> value='AM'>Approved Suggestion</option>



                  <option <? if ($isUserLevelFree) echo 'disabled="disabled"';  ?> value='DS'>Denied Suggestion</option>
            ...
            echo "<a href = \"".home_url()."/wp-admin/admin.php?page=woo-refyn-search&tab=machine-learning&page1=$last\"> Previous </a> |";



            echo "<a href = \"".home_url()."/wp-admin/admin.php?page=woo-refyn-search&tab=machine-learning&page1=$page1\"> Next  </a>";
		...
		<?php do_action( $this->plugin_name . '-' . trim( $form_key ) . '_settings_end' ); ?>
            ...
            <p class="submit">
                    ...
                    <input type="submit" value="<?php _e('Save changes', 'refyn'); ?>" class="button button-primary" name="bt_save_settings" />



                    <input type="submit" name="bt_reset_settings" class="button" value="<?php _e('Reset Settings', 'refyn'); ?>"  />
		...
		/*
		$get_user_data['user_level']='pro';



		$get_user_data['usage_exceed'] = date("Y-m-d", strtotime("+30 days"));



		$get_user_data['usage_counter']=20;
		*/
...
//		echo "user_data=$user_data";
		...
		<hr>
			...
			<b>USER LEVEL </b>: <? echo $get_user_data['user_level']; 



					   $isUserLevelFree = $get_user_data['usage_exceed'];
				...
				if ($isUserLevelFree){ 
					...
					echo '<a target="_blank" href="https://refyn.org/licenses/"><font color="red">Usage or 30 Days Limit Exceeded. Get Unlimited!</font></a>';
				...
				echo $get_user_data['usage_counter'];
			...
			&nbsp;|&nbsp;<b>MEMBER SINCE </b>: <? 
				...
				echo $get_user_data['registration_date']; 
...
$refyn_search_admin_interface = new Refyn_Search_Admin_Interface();
...
add_action('custom_options_css','custom_option_css_function');



function custom_option_css_function(){
	...
	.ac_over {
		...
		background-color: <?php echo $setting_options['refyn_search_background_hover_color']; ?>;



		color: <?php echo $setting_options['refyn_search_text_hover_color']; ?>;
