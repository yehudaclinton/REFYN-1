/**
 * REFYNrevThemes Typography Live Preview
 * 2011-10-07.
 *
...
(function ($) {
  ...
  refynrevTypographyPreview = {
    ...
    loadPreviewButtons: function () {
     ...
     refynrevTypographyPreview.handleEvents();
    ...
    handleEvents: function () {
    	$(document).on( 'click', 'a.refynrev-ui-typography-preview-button', function () {
    		refynrevTypographyPreview.generatePreview( $( this ) );
    		return false;
    	});
    	
    	$(document).on( 'click', 'a.preview_remove', function () {
    		refynrevTypographyPreview.closePreview( $( this ) );
    		return false;
    	});
    }, 
     ...
     */
     ...
     closePreview: function ( target ) {
		target.parents( '.section' ).find( '.refynrev-ui-typography-preview-button .refresh' ).removeClass( 'refresh' );
    ...
    generatePreview: function ( target ) {
    	...
    	var controls = target.parents( '.refynrev-ui-typography-control' );
    	
    	var sizeSelector = '.refynrev-ui-typography-size';
    	
    	var fontSize = controls.find( sizeSelector ).val();
    	
    	var fontFace = controls.find( '.refynrev-ui-typography-face' ).val();
    	var fontStyle = controls.find( '.refynrev-ui-typography-style' ).val();
    	var fontColor = controls.find( '.refynrev-ui-typography-color' ).val();
    	...
    	previewHTML = $( '<div />' ).addClass( 'typography-preview-container' ).html( previewHTMLInner ).append( '<a href="#" class="preview_remove refyn-plugin-ui-delete-icon">&nbsp;</a>' );
    	...
    	controls.find( '.refynrev-ui-typography-preview-button span' ).addClass( 'refresh' );
    }

   
  }; // End refynrevTypographyPreview Object // Don't remove this, or the sky will fall on your head.

/*-----------------------------------------------------------------------------------*/
/* Execute the above methods in the refynrevTypographyPreview object.
/*-----------------------------------------------------------------------------------*/
  
	$(document).ready(function () {

		refynrevTypographyPreview.loadPreviewButtons();
